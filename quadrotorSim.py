...
# Initial condition: all states start at zero
X0 = np.zeros(16)
...
# Convert to positive altitude for plotting
altitud = -z 

# Position and attitude plots
...
plt.plot(t, T_res[0], label='T1')
plt.plot(t, T_res[1], '--', label='T2')
plt.plot(t, T_res[2], ':', label='T3')
plt.plot(t, T_res[3], '-.', label='T4')
plt.title('Esfuerzo de Control (Empuje de Rotores)')
plt.xlabel('Tiempo (s)')
plt.ylabel('Empuje (N)')
plt.grid()
plt.legend()
